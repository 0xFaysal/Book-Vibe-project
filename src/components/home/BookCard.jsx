...
function BookCard({ data }) {
    const { bookId, image, tags, bookName, author, category, rating } = data;

    return (
        <Link to={`/${bookId}`}>
            <div className='p-6 border rounded-2xl'>
                <div className='bg-black-5 grid place-items-center py-8 rounded-2xl'>
                    <img className='h-[10.375rem]' src={image} alt={bookName} />
                </div>
                ...
                <h3 className='text-2xl font-bold text-black-full mt-4'>
                    {bookName}
                </h3>
                <p className='text-base font-medium text-black-80 mt-4'>
                    By : {author}
                </p>
                <hr className='my-5 border-dashed divide-dashed' />
